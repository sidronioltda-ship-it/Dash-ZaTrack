...
const pageRoutes = ["/", "/tracking"] as const;
type PageRoute = (typeof pageRoutes)[number];

function isPageRoute(href: string): href is PageRoute {
  return (pageRoutes as readonly string[]).includes(href);
}
    ...
    <aside className="glass-panel fixed inset-y-4 left-4 z-30 hidden w-[280px] overflow-hidden rounded-[2rem] lg:flex lg:flex-col">
      ...
      <ScrollArea className="relative flex-1 px-3 py-4">
        <div className="space-y-1">
          {navItems.map((item) => {
            const isActive = isPageRoute(item.href)
              ? item.href === "/"
                ? pathname === "/"
      ...
      <div className="relative m-3 rounded-[1.75rem] border border-white/10 bg-[#202020]/80 p-4">
        ...
        <p className="text-sm font-medium">Tracking ativo</p>
        <p className="mt-1 text-xs leading-5 text-muted-foreground">
          Monitoramento em tempo real de campanhas, leads e conversoes.
        </p>
